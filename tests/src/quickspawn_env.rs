...
use ict_rs::quickspawn::{ContractRegistry, QuickSpawnManager, SpawnedChain, SpawnedChainSet};
...
impl QuickSpawnEnv {
    ...
    pub async fn spawn(
        ...
        // The image/binary info comes from the snapshot — we use defaults
        // that match the bootstrap environment.
...
impl Drop for QuickSpawnEnv {
    fn drop(&mut self) {
        ...
        match tokio::runtime::Handle::try_current() {
            ...
            Err(_) => tracing::warn!(
                "QuickSpawnEnv drop: no tokio runtime active, container may leak"
            ),
        }
    }
}
...
impl QuickSpawnMultiEnv {
    ...
    pub async fn spawn_multi(
        ...
        let primary = chain_set.primary().ok_or_else(|| {
            anyhow::anyhow!("Multi-node snapshot has no validators")
        })?;
