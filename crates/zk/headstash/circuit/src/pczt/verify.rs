...
impl super::Action {
    ...
    pub fn verify_cv_net(&self) -> Result<(), VerifyError> {
        let spend_value = self.spend().v.ok_or(VerifyError::MissingValue)?;
        let output_value = self.output().v.ok_or(VerifyError::MissingValue)?;
        let rcv = self
            .rcv
            .clone()
...
impl super::Spend {
    ...
    fn fvk_for_validation<'a>(
        ...
        match (expected_fvk, self.fvk.as_ref(), self.v.as_ref()) {
    ...
    pub fn verify_nullifier(
        ...
        let note = Note::from_parts(
            self.nd.ok_or(VerifyError::MissingRecipient)?,
            self.v.ok_or(VerifyError::MissingValue)?,
            self.fdi.ok_or(VerifyError::MissingValue)?,
            self.recipient.ok_or(VerifyError::MissingRecipient)?,
            self.esk.ok_or(VerifyError::MissingRecipient)?,
...
impl super::Output {
    ...
    pub fn verify_note_commitment(&self, spend: &super::Spend) -> Result<(), VerifyError> {
        let note = Note::from_parts(
            self.nd.ok_or(VerifyError::MissingValue)?,
            self.v.ok_or(VerifyError::MissingValue)?,
            self.fdi.ok_or(VerifyError::MissingValue)?,
            self.recipient.ok_or(VerifyError::MissingValue)?,
            self.esk.ok_or(VerifyError::MissingValue)?,
