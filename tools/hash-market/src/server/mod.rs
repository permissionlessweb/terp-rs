...
use crate::ve::{SignedVoteExtension, VoteExtensionHandler};
use axum::{
    extract::State,
    http::StatusCode,
...
pub struct AppState {
    ...
    pub chain_id: String,
    /// Latest data per (chain_uid, algo) from all providers.
    pub provider_data: RwLock<HashMap<ProviderKey, ProviderData>>,
    /// Provider metadata for status reporting.
    pub provider_status: RwLock<Vec<ProviderStatus>>,
    /// Current block height (updated by CometBFT callbacks).
...
/// terpd calls `GET {sidecar_url}/vote-extension` every block during the
...
async fn vote_extension_get(
    ...
    // Return the most recently updated provider's data.
    let entry = store
        .values()
...
async fn extend_vote(
    ...
    let data = if let Some(chain_uid) = &req.chain_uid {
        ...
        store.values().next().ok_or(StatusCode::SERVICE_UNAVAILABLE)?
