...
DEX_CONTRACT_ART="$CRATES/terp-rs/contracts/revenue/private-dex/artifacts"
DEST="$ART/cw_headstash.wasm"
DEST2="$CONTRACT_ART/cw_headstash.wasm"
DEX_DEST="$ART/cw_private_dex.wasm"  # artifact label kept for Daemon find_wasm
...
prepare_private_dex() {
  ...
  log "cargo build -p cw-private-dex --target wasm32-unknown-unknown --release --manifest-path "$CRATES/terp-rs/Cargo.toml" (guest graph, no zk-api)"
  (
    cd "$HS"
    # Default features: no zk-api → stock wasmd compatible (mock_verify lab)
    cargo build -p cw-private-dex --target wasm32-unknown-unknown --release --manifest-path "$CRATES/terp-rs/Cargo.toml"
  )

  RAW="$CRATES/terp-rs/target/wasm32-unknown-unknown/release/cw_private_dex.wasm"
  [ -f "$RAW" ] || fail "missing raw wasm after cargo build: $RAW"
