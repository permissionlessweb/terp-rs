...
- Generator wiring is the largest win: binary now imports `scripts::ibc`, builds `PredictedWorld`, runs `check_invariants` fail-closed for **routing/lookup/meta**, and exposes clap `generate|validate|compare`. Local premine/`hop_count: route.len()`/`dead_clients 0..31` are gone.
